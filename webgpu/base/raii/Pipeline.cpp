...
GenericRenderPipeline::GenericRenderPipeline(WGPUDevice device,
    const ShaderModule& vertex_shader,
    const ShaderModule& fragment_shader,
    const VertexBufferInfos& vertex_buffer_infos,
    const FramebufferFormat& framebuffer_format,
    const BindGroupLayouts& bind_group_layouts,
    const std::vector<std::optional<WGPUBlendState>>& blend_states)
    : GenericRenderPipeline(device,
          vertex_shader,
          fragment_shader,
          vertex_buffer_infos,
          framebuffer_format,
    ...
    : m_framebuffer_format { framebuffer_format }
{
    ...
    std::transform(
        bind_group_layouts.begin(), bind_group_layouts.end(), std::back_insert_iterator(bind_group_layout_handles), [](const BindGroupLayout* layout) {
    ...
    std::transform(vertex_buffer_infos.begin(), vertex_buffer_infos.end(), std::back_insert_iterator(layouts), [](const util::SingleVertexBufferInfo& info) {
        return info.vertex_buffer_layout();
    });
    ...
    // stripIndexFormat is only meaningful for strip topologies; WebGPU requires it be
    // Undefined for non-strip topologies, otherwise validation fails.
    pipeline_desc.primitive.stripIndexFormat = (topology == WGPUPrimitiveTopology_TriangleStrip || topology == WGPUPrimitiveTopology_LineStrip)
        ? WGPUIndexFormat::WGPUIndexFormat_Uint16
